...
| `accesos` | `v_mdg_accesos` (SHP gen) | Citywide, 200k+ points | `CALLE`/`NOM_CALLE`, `NUM_PUERTA` (+ `PUERTA`/`NRO_PUERTA`/`NRO` legacy fallbacks), `PADRON` | Official address points, joined by padrón. `NUM_PUERTA` is the actual door-number field; the others never matched anything (pre-existing bug fixed Task 7). |
